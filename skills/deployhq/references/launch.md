...
| `--accept-cost` | Acknowledge the monthly VPS cost. **Required** for non-interactive VPS provisioning |
| `--branch` | Branch to deploy (default: repo default) |
| `--project` | Existing project permalink to reuse (skips project creation) |
| `--cleanup-on-failure` | Delete the provisioned server if the deploy fails (prevents orphaned billable resources) |
...
- **Billable guardrail:** a Managed VPS is billable; in non-interactive mode it is **never** provisioned without `--accept-cost` (`--yes` alone is not enough). Static Hosting is safe under `--yes`.
- **`--dry-run`** emits `{would, requires, warning}` and makes no changes — use it to preview cost and confirm before a billable run.
...
| `accept_cost_required` | Billable VPS requested non-interactively without `--accept-cost` — re-run with `--accept-cost` |
...
| `deploy_failed` | Provisioning succeeded but the deploy failed — the (billable) server is named with its teardown command; `--cleanup-on-failure` removes it automatically |
